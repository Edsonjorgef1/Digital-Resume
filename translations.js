const translations = {
    pt: {
        title: "CV Profissional - Edson Jorge Francisco",
        role: "Gestor de TI & Engenheiro Informático",
        sections: {
            experience: "Experiência Profissional",
            education: "Educação e Formação",
            skills: "Competências",
            projects: "Projetos",
            achievements: "Conquistas e Avaliações",
            references: "Referências",
            techStack: "Stack Tecnológica",
            personalProjects: "Projetos Pessoais e Conquistas",
            downloadCV: "Baixar CV em PDF"
        },
        skills: {
            languages: "Línguas",
            digital: "Competências Digitais",
            interpersonal: "Competências Interpessoais"
        },
        achievements: {
            gallup: "Avaliação de Pontos Fortes (Gallup, 2024)",
            high5: "Certificado HIGH5 Strengths (2025)",
            otherAchievements: "Outras Conquistas",
            personalProjects: "Projetos Pessoais e Conquistas",
            facim2023: "2023: Participação na FACIM - Expositor de soluções digitais em representação da marca FAMS - Software para Agronegócios"
        },
        footer: "© 2025 Edson Jorge Francisco. Todos os direitos reservados."
    },
    en: {
        title: "Professional CV - Edson Jorge Francisco",
        role: "IT Manager & Computer Engineer",
        sections: {
            experience: "Professional Experience",
            education: "Education and Training",
            skills: "Skills",
            projects: "Projects",
            achievements: "Achievements and Assessments",
            references: "References",
            techStack: "Tech Stack",
            personalProjects: "Personal Projects & Accomplishments",
            downloadCV: "Download CV in PDF"
        },
        skills: {
            languages: "Languages",
            digital: "Digital Skills",
            interpersonal: "Interpersonal Skills"
        },
        achievements: {
            gallup: "Strengths Assessment (Gallup, 2024)",
            high5: "HIGH5 Strengths Certificate (2025)",
            otherAchievements: "Other Achievements",
            personalProjects: "Personal Projects & Accomplishments",
            facim2023: "2023: Participation in FACIM - Digital Solutions Exhibitor representing FAMS - Agribusiness Software"
        },
        footer: "© 2025 Edson Jorge Francisco. All rights reserved."
    },
    es: {
        title: "CV Profesional - Edson Jorge Francisco",
        role: "Gerente de TI & Ingeniero Informático",
        sections: {
            experience: "Experiencia Profesional",
            education: "Educación y Formación",
            skills: "Competencias",
            projects: "Proyectos",
            achievements: "Logros y Evaluaciones",
            references: "Referencias",
            techStack: "Stack Tecnológico",
            personalProjects: "Proyectos Personales y Logros",
            downloadCV: "Descargar CV en PDF"
        },
        skills: {
            languages: "Idiomas",
            digital: "Competencias Digitales",
            interpersonal: "Competencias Interpersonales"
        },
        achievements: {
            gallup: "Evaluación de Fortalezas (Gallup, 2024)",
            high5: "Certificado HIGH5 Strengths (2025)",
            otherAchievements: "Otros Logros",
            personalProjects: "Proyectos Personales y Logros",
            facim2023: "2023: Participación en FACIM - Expositor de soluciones digitales representando a FAMS - Software para Agronegocios"
        },
        footer: "© 2025 Edson Jorge Francisco. Todos los derechos reservados."
    },
    de: {
        title: "Professioneller Lebenslauf - Edson Jorge Francisco",
        role: "IT-Manager & Informatikingenieur",
        sections: {
            experience: "Berufserfahrung",
            education: "Ausbildung",
            skills: "Fähigkeiten",
            projects: "Projekte",
            achievements: "Erfolge und Bewertungen",
            references: "Referenzen",
            techStack: "Tech-Stack",
            personalProjects: "Persönliche Projekte & Erfolge",
            downloadCV: "Lebenslauf als PDF herunterladen"
        },
        skills: {
            languages: "Sprachen",
            digital: "Digitale Fähigkeiten",
            interpersonal: "Zwischenmenschliche Fähigkeiten"
        },
        achievements: {
            gallup: "Stärkenbewertung (Gallup, 2024)",
            high5: "HIGH5 Strengths Zertifikat (2025)",
            otherAchievements: "Andere Erfolge",
            personalProjects: "Persönliche Projekte & Erfolge",
            facim2023: "2023: Teilnahme an der FACIM - Aussteller für digitale Lösungen im Namen von FAMS - Software für Agribusiness"
        },
        footer: "© 2025 Edson Jorge Francisco. Alle Rechte vorbehalten."
    }
}; 